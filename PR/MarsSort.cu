...
__global__ void
bitonicKernel( void* d_rawData, int totalLenInBytes, cmp_type_t* d_R,
 unsigned int numRecords, int k, int j)
{
	int bx = blockIdx.x;
	int by = blockIdx.y;
	int tid = threadIdx.x;
		  ...
		  )
{
	...
	// endTimer("bitonicSort", &sortTimer);
	// startTimer(&postTimer);
